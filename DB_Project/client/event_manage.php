...
<html>
<head>
	<title>Urban Management- Add/Edit Event </title>
	<style type="text/css">
		a:hover{
			font-size: 24px;
		}
...
<body background="..\image\concrete1.jpg">
	...
	<div id="add">
		<form action="event_manage.php#add" method="POST">
			<table align="center" cellspacing="20px">
				...
				<tr><td>Artist: 
					<select name="a_eventartist" >
						<?php
							 $conn = mysqli_connect($servername, $username, $password, $dbname);
							 $read_venue = "SELECT * FROM artist ";
							$result_read_venue = mysqli_query($conn, $read_venue);
							if(mysqli_num_rows($result_read_venue)>0){
								while($row = mysqli_fetch_array($result_read_venue, MYSQLI_ASSOC)){
				...
				<tr><td>Sponser: 
					<select name="a_eventsponser" >
						<?php
							 $conn = mysqli_connect($servername, $username, $password, $dbname);
							 $read_venue = "SELECT * FROM sponser ";
							$result_read_venue = mysqli_query($conn, $read_venue);
							if(mysqli_num_rows($result_read_venue)>0){
								while($row = mysqli_fetch_array($result_read_venue, MYSQLI_ASSOC)){
									echo "<option value='".$row['Sponser_name']."'>".$row['Sponser_name']."</option>";
								}
							}
						?>
					</select>
			...
			<table align="center" cellspacing="20px">
				...
				<tr><td>Event: 
					...
					<option value="">Select</option>
						<?php
							$conn = mysqli_connect($servername, $username, $password, $dbname);
							$read_event = "SELECT * FROM event_details INNER JOIN venue_details ON event_details.VenueID = venue_details.VenueID AND event_details.USERID='$uid'";
	...
	<?php
		...
		if (isset($_POST['addevent'])) {
			...
			$eartist=$_POST['a_eventartist'];
			$esponser=$_POST['a_eventsponser'];
			...
			$read_artist_id="SELECT Artist_id FROM artist WHERE Artist_Name='$eartist'";
			$read_sponser_id="SELECT Sponser_id FROM sponser WHERE Sponser_name='$esponser'";
			$result_read_venue_id = mysqli_query($conn, $read_venue_id);
			$result_read_artist_id = mysqli_query($conn, $read_artist_id);
			$result_read_sponser_id = mysqli_query($conn, $read_sponser_id);
			if($result_read_venue_id){
				while($row = mysqli_fetch_array($result_read_venue_id, MYSQLI_ASSOC) ){
					$vid=$row['VenueID'];
					while($row1 = mysqli_fetch_array($result_read_artist_id, MYSQLI_ASSOC)){
						$aid=$row1['Artist_id'];
						while($row3 = mysqli_fetch_array($result_read_sponser_id, MYSQLI_ASSOC)){

							$sid=$row3['Sponser_id'];
							//Insert event
							$insert_event = "INSERT INTO event_details (EventName, EventDate, EventTime, EventCategory, EventDescription, EventTicketPrice, EventTicketTotal, VenueID, UserID,Artist_id,Sponsor_id) VALUES ('$ename', '$edate', '$etime', '$ecategory', '$edescription', $eprice, $etotal, '$vid', '$uid', '$aid','$sid')";
		...
		if (isset($_POST['deleteevent'])) {
			...
			//Read event id
			$read_event_id = "SELECT EventID FROM event_details WHERE EventName='$selectname' AND UserID='$uid'";
