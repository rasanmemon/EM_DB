 <?php
	...
	if (isset($_POST['login'])) {
		$uid = $_POST['userid'];
		$upass = $_POST['userpass'];
		$upass2 = hash('sha512',$_POST['userpass']);
		$conn = mysqli_connect($servername, $username, $password, $dbname);
		$read_DB = "SELECT * FROM user_details WHERE UserID='$uid' AND (UserPassword='$upass' OR UserPassword='$upass2')";
		...
		if(mysqli_num_rows($result)>0){
			while($row = mysqli_fetch_array($result, MYSQLI_ASSOC)){
				...
				if (strcmp($upass, $row['UserPassword']) == 0 || strcmp($upass2, $row['UserPassword']) == 0) { 
    				session_start();
...
<body background="..\image\concrete1.jpg" >
	...
	<!-- <form method="POST" class="registerform" action="login_register.php">
		...
		<br><br>
		<input type="text" name="usertype" value="Guest" placeholder="User Type" readonly>
		<br><br>
