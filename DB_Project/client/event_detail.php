					...
					while($row = mysqli_fetch_array($result_read_ticket_info, MYSQLI_ASSOC)){
						...
						//Else, update ticket sold, then insert booking detail
							...
							if($cop==''){
								...
								$aid= "SELECT Artist_id from artist where Artist_id='$cop' ";
								$result_cop = mysqli_query($conn, $aid);
								if($result_cop){
									while($row = mysqli_fetch_array($result_cop, MYSQLI_ASSOC)){
										$artist=$row['Artist_id'];
										$insert_weak="INSERT INTO weakentity (Guest_id,Artist_id,EventID,Relation) VALUES('$uid','$artist','$eid','$rel')";
										$result_insert_waek = mysqli_query($conn, $insert_weak);
									}
									$currentsoldticket = $ticketsold+1;
									$update_ticket_sold = "UPDATE event_details SET EventTicketSold=$currentsoldticket WHERE EventID='$eid'";
								$result_update_ticket_sold = mysqli_query($conn, $update_ticket_sold);
...
<html>
<head>
	<title>Urban Management - Event Detail/Search Event</title>
	<style>
		a:hover{
			font-size: 24px;
		}
...
<body background="image\concrete1.jpg">
	...
	<div class="content" align="center">
		<?php
			...
			$read_DB = "SELECT * FROM event_details INNER JOIN venue_details ON event_details.VenueID = venue_details.VenueID INNER JOIN artist  on event_details.Artist_id=artist.Artist_id INNER JOIN sponser on event_details.Sponsor_id=Sponser.Sponser_id WHERE  event_details.EventName LIKE '$searchkey%'   ";
			$result = mysqli_query($conn, $read_DB);
			// $aid="SELECT Artist_Name from artist join event_details where event_details.Artist_id=artist.Artist_id";
			// $result1=mysqli_query($conn,$aid);
			// $sid="SELECT Sponser_Company from sponser join event_details where event_details.Sponsor_id=Sponser.Sponser_id";
			// $result2=mysqli_query($conn,$sid);
			...
			if(mysqli_num_rows($result)>0){
    			...
    			while($row = mysqli_fetch_array($result, MYSQLI_ASSOC)){
					...
					<li><b>Artist: </b>".$row['Artist_Name']."</li>
					<li><b>Sponser:</b> ".$row['Sponser_Company']."</li>
					<li>Coupon: <input type=text name='coupon'/></li>
					<li>Relation: <input type=text name='relation'/></li>
					</ul></span></td></tr>";
