...
<body background="..\image\conrete1.jpg">
	...
	<div id="view" align="center">
		...
		<hr>
		<form action="sponser_manage_view.php" method="POST" style="font-size: 20px;">
		...
		<table align="center" cellpadding="20px" cellspacing="6px">
			...
			<?php
				if (isset($_POST['ascending'])) {
					...
					$read_user = "SELECT * FROM sponser ORDER BY Sponser_name ASC";
					...
					if(mysqli_num_rows($result_read_user)>0){
						while($row = mysqli_fetch_array($result_read_user, MYSQLI_ASSOC)){
							...
							echo "<tr>";
							echo "<td>".$count."</td>";					
							echo "<td>".$row['Sponser_id']."</td>";
							echo "<td>".$row['Sponser_name']."</td>";
							echo "<td>".$row['Sponser_email']."</td>";
							echo "<td>".$row['Sponser_phoneno']."</td>";
							echo "<td>".$row['Sponser_Company']."</td>";
							echo "<tr>";
						}
					}
				}
					...
					$count=0;
					$conn = mysqli_connect($servername, $username, $password, $dbname);
					$read_user = "SELECT * FROM sponser ORDER BY Sponser_name DESC";
					$result_read_user = mysqli_query($conn, $read_user);
					if(mysqli_num_rows($result_read_user)>0){
						while($row = mysqli_fetch_array($result_read_user, MYSQLI_ASSOC)){
							$count=$count+1;
					...
					$count=0;
					$conn = mysqli_connect($servername, $username, $password, $dbname);
					$read_user = "SELECT * FROM sponser ORDER BY Sponser_id ASC";
					$result_read_user = mysqli_query($conn, $read_user);
					if(mysqli_num_rows($result_read_user)>0){
						while($row = mysqli_fetch_array($result_read_user, MYSQLI_ASSOC)){
							$count=$count+1;
